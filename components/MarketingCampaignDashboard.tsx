...
function localPreviewPath(path: string, previewPackCode: string) {
  if (!path || !previewPackCode) return path;
  const url = new URL(path, "http://lorewise.local");
  url.searchParams.set("anteprima", previewPackCode === "GW-PROMO-FESTE-NEXUS-2026" ? "feste" : "halloween");
...
export function MarketingCampaignDashboard({ previewPackCode = "" }: { previewPackCode?: string }) {
  ...
  const emailPreviewPath = localPreviewPath(draft.actionUrl, previewPack?.code ?? "");
