...
for (const file of publicFiles) {
  ...
  if (extension === ".png" && !certificatePngs.has(relative)) failures.push(`${relative}: PNG sorgente rimasto nell'area pubblica`);
...
else if (fs.statSync(serverEntry).size > 15 * 1024 * 1024) failures.push("dist/server/index.js supera 15 MB");
if (publicBytes > 200 * 1024 * 1024) failures.push(`public supera 200 MB (${(publicBytes / 1024 / 1024).toFixed(2)} MB)`);
if (clientBytes > 220 * 1024 * 1024) failures.push(`dist/client supera 220 MB (${(clientBytes / 1024 / 1024).toFixed(2)} MB)`);
...
console.log(
  ...
  `soltanto i due PNG necessari ai certificati, nessun archivio privato o file pubblico oltre 3 MB.`,
);
