...
const pattoVillains = [
  { name: "Azhrek", title: "Prefetto delle Catene", image: "/games/the-wound-remembers-il-patto-delle-ceneri/azhrek-prefetto-delle-catene-v1.webp", alt: "Azhrek, Prefetto delle Catene", description: "Amministra le prigioni imperiali e trasforma ogni via di fuga in un cappio. Le sue stesse catene possono però diventare l’arma che lo condanna." },
  { name: "Malcera", title: "Nutrice delle Piaghe", image: "/games/the-wound-remembers-il-patto-delle-ceneri/malcera-nutrice-delle-piaghe-v1.webp", alt: "Malcera, Nutrice delle Piaghe", description: "Coltiva i corpi dell’esercito della Corte e converte il campo in un vivaio velenoso. Fermarla significa scegliere che cosa bruciare e chi liberare." },
  { name: "Gorvath", title: "Boia del Primo Patto", image: "/games/the-wound-remembers-il-patto-delle-ceneri/gorvath-boia-del-primo-patto-v1.webp", alt: "Gorvath, Boia del Primo Patto", description: "Pronuncia condanne prima di eseguirle. Proteggere il marchiato non è soltanto sopravvivere: è spezzare i sigilli che alimentano la sua mannaia." },
  { name: "Ishara", title: "Custode dei Volti", image: "/games/the-wound-remembers-il-patto-delle-ceneri/ishara-custode-dei-volti-v1.webp", alt: "Ishara, Custode dei Volti", description: "Conserva identità, ricordi e maschere confiscate. Copia le capacità della compagnia e costringe ogni eroe ad affrontare una versione rubata di sé." },
];
...
export default async function GameProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isPattoDelleCeneri = project.slug === "the-wound-remembers-il-patto-delle-ceneri";
  ...
  if (isPattoDelleCeneri) return <PattoCeneriDossier />;
  if (project.slug === "sandbox") return <SandboxDossier />;
  return <main className={`game-dossier-page game-dossier-${project.statusTone}${isPattoDelleCeneri ? " game-dossier-patto-ceneri" : ""}${isDemonMatch ? " game-dossier-demon-match" : ""}`}>
    ...
    <article>
      <header className="game-dossier-hero">
        ...
        <div className={`shell game-dossier-hero-copy${isTheWoundRemembers ? ` ${styles.woundHeroCopy}` : ""}`}>
          ...
          <div className="game-dossier-hero-actions">
            {isTheWoundRemembers ? <>{project.publicUrl ? <FunnelLink className="game-hero-primary" href={project.publicUrl} eventName="play_cta_click" source="landing_hero">Gioca ora <span aria-hidden="true">→</span></FunnelLink> : null}<TwrGameplayTrailer className="game-hero-secondary" /></> : isPattoDelleCeneri ? <><a className="game-hero-primary" href="#storia">Scopri la storia</a><a className="game-hero-secondary" href="#personaggi">Incontra il Rifugio</a></> : isDemonMatch ? <><a className="game-hero-primary" href="#come-si-gioca">Scopri il gameplay</a><a className="game-hero-secondary" href="#edizione-windows">Stato Android</a></> : <>{project.publicUrl && project.publicAction ? <a className="game-hero-primary" href={project.publicUrl} target="_blank" rel="noopener noreferrer">{project.publicAction} <span aria-hidden="true">↗</span></a> : null}{project.windowsOffer ? <a className="game-hero-secondary" href="#edizione-windows">Edizione Windows · {project.windowsOffer.launchPrice}</a> : null}</>}
          </div>
        </div>
      </header>

      <nav className="shell game-dossier-index" aria-label={`Percorso rapido di ${project.title}`}>
        <a href={isPattoDelleCeneri ? "#storia" : "#panoramica"}>{isPattoDelleCeneri ? "Storia" : "Panoramica"}</a>
        {project.playFlow ? <a href="#come-si-gioca">Gameplay</a> : null}
        <a href={isPattoDelleCeneri ? "#personaggi" : "#contenuti"}>{isPattoDelleCeneri ? "Personaggi" : "Contenuti"}</a>
        {isPattoDelleCeneri ? <a href="#villain">Villain</a> : project.latestUpdate ? <a href="#sviluppo">Aggiornamenti</a> : null}
        <a href={isPattoDelleCeneri ? "#in-lavorazione" : "#edizione-windows"}>{isPattoDelleCeneri ? "In lavorazione" : "Accesso"}</a>
      </nav>

      <section className="shell game-dossier-summary" id="panoramica" aria-labelledby="game-summary-title"><div><p className="eyebrow">{isPattoDelleCeneri ? "Una nuova cronaca" : "Progetto GiWise Studio"}</p><h2 id="game-summary-title">{project.title}</h2><p>{project.summary}</p></div>{isPattoDelleCeneri ? <dl><div><dt>Genere</dt><dd>GDR tattico dark fantasy</dd></div><div><dt>Compagnia</dt><dd>Quattro membri, nessun ruolo imposto</dd></div><div><dt>Esperienza</dt><dd>Storia, esplorazione e battaglie a turni</dd></div><div><dt>Scelte</dt><dd>Alleanze e conseguenze persistenti</dd></div></dl> : <dl><div><dt>Stato</dt><dd>{project.status}</dd></div><div><dt>Versione</dt><dd>{project.version}</dd></div><div><dt>Ultimo aggiornamento</dt><dd>{project.latestUpdate?.date ?? "In sviluppo · data pubblica non definita"}</dd></div><div><dt>Lingua</dt><dd>{project.languages.join(" · ")}</dd></div><div><dt>Tipo</dt><dd>{project.kind}</dd></div><div><dt>Piattaforme</dt><dd>{project.platforms.join(" · ")}</dd></div><div><dt>Accesso</dt><dd>{project.access}</dd></div><div><dt>Prezzo</dt><dd>{project.price}</dd></div></dl>}</section>

      <section className="shell game-dossier-story" id={isPattoDelleCeneri ? "storia" : undefined} aria-labelledby="game-story-title"><span>01</span><div><p className="eyebrow">{isPattoDelleCeneri ? "Dopo The Wound Remembers" : "Identità del progetto"}</p><h2 id="game-story-title">{isPattoDelleCeneri ? "La storia continua nelle ceneri." : "Il gioco"}</h2><p>{project.description[0]}</p>{project.description.length > 1 ? <div className="game-story-continuation">{project.description.slice(1).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div> : null}</div></section>
      ...
      {project.protagonistReveal ? <section className="game-protagonist-reveal" id="protagonisti" aria-labelledby="game-protagonist-title">
        <div className="shell game-protagonist-intro">
          ...
          <div><p className="eyebrow">{project.protagonistReveal.eyebrow}</p><h2 id="game-protagonist-title">{project.protagonistReveal.title}</h2><p>{project.protagonistReveal.introduction}</p>{isDemonMatch ? <Link href="/vip-zone?area=games&game=demon-match-three#vip-demon-match">Apri l’anteprima nell’Area VIP <span aria-hidden="true">→</span></Link> : null}</div>
        </div>
      ...
      {isPattoDelleCeneri ? <section className="patto-villains" id="villain" aria-labelledby="patto-villains-title"><div className="shell"><header><p className="eyebrow">Le Quattro Braci del Trono</p><h2 id="patto-villains-title">La Corte ha nuovi volti.</h2><p>Quattro comandanti alimentano il potere dell’Imperatore del Rogo. Puoi affrontarli, spezzare ciò che li sostiene o trovare il modo di rivolgere la loro autorità contro la Corte.</p></header><div className="patto-villain-grid">{pattoVillains.map((villain) => <article key={villain.name}><figure><Image src={villain.image} alt={villain.alt} width={1024} height={1536} unoptimized /></figure><div><span>{villain.title}</span><h3>{villain.name}</h3><p>{villain.description}</p></div></article>)}</div></div></section> : null}
      ...
      {project.playFlow ? <section className="game-play-flow" id="come-si-gioca" aria-labelledby="game-play-title"><div className="shell"><header><p className="eyebrow">{isPattoDelleCeneri ? "La tua compagnia, le tue conseguenze" : isDemonMatch ? "Dall’obiettivo alla ricompensa" : "Una partita, dall’inizio alla cicatrice"}</p><h2 id="game-play-title">Come si gioca.</h2>{isPattoDelleCeneri ? <p className="game-play-flow-note">Prepara il Patto, esplora, combatti e torna al Rifugio: ogni fase alimenta la storia successiva e rende la compagnia sempre più personale.</p> : isDemonMatch ? <p className="game-play-flow-note">Cinque catture reali della build Android corrente, mostrate interamente e senza ritagli: mappa, briefing, preparazione dei potenziamenti, fusione e cascata. Il dossier resta privo di spoiler sui protagonisti.</p> : null}</header><ol>{project.playFlow.map((step) => <li key={step.number}><div className="game-play-flow-image"><Image src={step.image} alt={step.imageAlt} width={2560} height={1440} unoptimized /></div><div><span>{step.number}</span><h3>{step.title}</h3><p>{step.description}</p></div></li>)}</ol></div></section> : null}

      <section className="game-dossier-features" id="contenuti" aria-labelledby="game-features-title"><div className="shell"><header><p className="eyebrow">Sistema e contenuti</p><h2 id="game-features-title">Cosa contiene.</h2></header><ol>{project.features.map((feature, index) => <li key={feature}><span>{String(index + 1).padStart(2, "0")}</span><strong>{feature}</strong></li>)}</ol></div></section>

      {!isPattoDelleCeneri ? <section className="shell game-known-issues" aria-labelledby="game-known-issues-title"><header><p className="eyebrow">Informazioni utili</p><h2 id="game-known-issues-title">Prima di iniziare.</h2><p>{project.mediaNote}</p></header><details><summary>Compatibilità e limiti attuali</summary><ol>{project.knownIssues.map((issue, index) => <li key={issue}><span>{String(index + 1).padStart(2, "0")}</span><p>{issue}</p></li>)}</ol></details></section> : null}

      {project.latestUpdate && !isPattoDelleCeneri ? <section className="game-development-journal" id="sviluppo" aria-labelledby="game-journal-title"><div className="shell">
        <header className="game-journal-heading"><div><p className="eyebrow">Aggiornamenti ufficiali</p><h2 id="game-journal-title">{isPattoDelleCeneri ? "Il nuovo Patto prende forma." : isDemonMatch ? "La griglia prende vita su Android." : "Il gioco continua."}</h2></div><p>Versioni, lavori in corso e stato delle piattaforme vengono pubblicati qui senza anticipazioni inventate o date non confermate.</p></header>
      ...
      {isPattoDelleCeneri ? <section className="patto-coming" id="in-lavorazione" aria-labelledby="patto-coming-title"><div className="shell"><p className="eyebrow">In lavorazione</p><h2 id="patto-coming-title">Il Patto sta aprendo le sue porte.</h2><p>Questo è il primo sguardo ufficiale al nuovo capitolo di The Wound Remembers. Il dossier accoglierà personaggi, luoghi e minacce quando saranno pronti per essere mostrati.</p><Link href="/cronache-del-nexus">Segui le novità dal Nexus <span aria-hidden="true">→</span></Link></div></section> : <section className="game-dossier-access" id="edizione-windows" aria-labelledby="game-access-title"><div className="shell"><div className="game-access-intro"><p className="eyebrow">{project.freeAccess ? "Accesso e partecipazione" : isDevelopmentOnly ? "Distribuzione in preparazione" : "Distribuzione ufficiale GiWise Studio"}</p><h2 id="game-access-title">{project.freeAccess ? "Entra nello sviluppo." : isDevelopmentOnly ? "In sviluppo." : "Scegli la tua edizione."}</h2><p>{project.freeAccess ? "Il gioco resta gratuito per tutti. Universe Pass sostiene il progetto e apre anticipazioni e occasioni di partecipazione, senza trasformarsi nel prezzo del download." : isDevelopmentOnly ? "La build resta privata finché piattaforme, account, pacchetto e condizioni di distribuzione non saranno verificati." : "Web, Windows e prossimamente Android: ogni versione viene presentata e distribuita direttamente da LoreWise Universe."}</p></div><div className={`game-editions${project.freeAccess ? " game-development-access" : ""}${isDevelopmentOnly ? " game-development-platforms" : ""}`}>
        {project.freeAccess ? <>
          ...
          <article className="game-edition game-edition-pass"><div><span>Universe Pass</span><strong>Più vicino allo studio</strong><p>L’abbonamento non sostituisce il prezzo del gioco. Offre contenuti editoriali e strumenti di partecipazione collegati al LoreWise ID.</p></div><ul>{project.passBenefits?.map((item) => <li key={item}>{item}</li>)}</ul><Link href="/account#vantaggi">Apri I miei vantaggi</Link></article>
