  ...
  return <main className={`game-dossier-page game-dossier-${project.statusTone}${isFuoriTrama ? " game-dossier-fuori-trama" : ""}${isDemonMatch ? " game-dossier-demon-match" : ""}`}>
    ...
    <article>
      ...
      <nav className="shell game-dossier-index" aria-label={`Percorso rapido di ${project.title}`}>
        ...
        {project.latestUpdate ? <a href="#sviluppo">Aggiornamenti</a> : null}
        <a href="#edizione-windows">Accesso</a>
      </nav>

      <section className="shell game-dossier-summary" id="panoramica" aria-labelledby="game-summary-title"><div><p className="eyebrow">Progetto GiWise Studio</p><h2 id="game-summary-title">{project.title}</h2><p>{project.summary}</p></div><dl><div><dt>Stato</dt><dd>{project.status}</dd></div><div><dt>Versione</dt><dd>{project.version}</dd></div><div><dt>Ultimo aggiornamento</dt><dd>{project.latestUpdate?.date ?? "In sviluppo · data pubblica non definita"}</dd></div><div><dt>Lingua</dt><dd>{project.languages.join(" · ")}</dd></div><div><dt>Tipo</dt><dd>{project.kind}</dd></div><div><dt>Piattaforme</dt><dd>{project.platforms.join(" · ")}</dd></div><div><dt>Accesso</dt><dd>{project.access}</dd></div><div><dt>Prezzo</dt><dd>{project.price}</dd></div></dl></section>

      <section className="shell game-dossier-story" aria-labelledby="game-story-title"><span>01</span><div><p className="eyebrow">Identità del progetto</p><h2 id="game-story-title">Il gioco</h2><p>{project.description[0]}</p>{project.description.length > 1 ? <details className="game-story-more"><summary>Approfondisci il progetto</summary>{project.description.slice(1).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</details> : null}</div></section>
      ...
      <section className="shell game-known-issues" aria-labelledby="game-known-issues-title"><header><p className="eyebrow">Informazioni utili</p><h2 id="game-known-issues-title">Prima di iniziare.</h2><p>{project.mediaNote}</p></header><details><summary>Compatibilità e limiti attuali</summary><ol>{project.knownIssues.map((issue, index) => <li key={issue}><span>{String(index + 1).padStart(2, "0")}</span><p>{issue}</p></li>)}</ol></details></section>
